...
class MyRepairList extends Component {
  constructor(props) {
    ...
    this.state = {
      query:search[pathname]?{...search[pathname]}:{menuFstate:"myRepairList"}
    }
  }
  /* 查询时向redux中插入查询参数 */
  queryHandler = (val) => {
    ...
    let columns = [
      {
        ...
        render: (text, record) => (
          ...
          <span>
            <Link to={{pathname: `/repairMgt/myRepairList/order/${record.rrpairOrderGuid}`,state : record}}>
              <Icon type="meh-o" style={{marginRight: 5}}/>指派
            </Link>
          </span>  :
          <span>
            <Link to={{pathname: `/repairMgt/myRepairList/detail/${record.rrpairOrderGuid}`,state : record}}>
