...
class DepreciateClassify extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      topClassInfo:{},
      modalProductQuery:{type:"02"},//选择产品搜索条件
      ...
      query: {type:'02',staticId:''}, // 查询条件(表格列表查询)
      tfClo: '', // 物资ID
  ...
  componentWillMount () {
    ...
    request(basicdata.queryStaticZcByCode,{
      body:querystring.stringify({tfClo:'depreciation'}),
      ...
      success: data => {  
        ...
        //获取资产分类列表
        this.getTreeData({tfClo:'depreciation'});
      },
      ...
      onOk: () => {
        ...
        if (selectedRowKeys.length) {
          request(basicdata.deleteAssetsTypeList,{
            body:querystring.stringify({type:"02",assetsRecordGuids: selectedRowKeys}),
            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
            success: data => {
              if (data.status) {
                message.success("操作成功！");
...
export default Form.create()(DepreciateClassify);
