/*
 * @Author: yuwei - 财政分类
 * @Date: 2018-06-22 09:37:32 
* @Last Modified time: 2018-06-22 09:37:32 
 */
...
class FinanceClassify extends Component {

  constructor(props) {
    super(props);
    this.state = {
      topClassInfo:{},
      modalProductQuery:{type:"03"},//选择产品搜索条件
      ...
      query: {type:'03',staticId:''}, // 查询条件(表格列表查询)
      tfClo: '', // 物资ID
  ...
  componentWillMount () {
    ...
    request(basicdata.queryStaticZcByCode,{
      body:querystring.stringify({tfClo:'finance'}),
      ...
      success: data => {  
        ...
        //获取资产分类列表
        this.getTreeData({tfClo:'finance'});
      },
  ...
  onSelect = (selectedKeys, info) => {
    ...
    this.setState({
      ...
      query:{type:'03',staticId:info.node.props.guId}
    })
    if(info.selected){
      if(this.refs.table){
        this.refs.table.fetch({type:'03',staticId:info.node.props.guId})
      }
    }else{
      ...
      onOk: () => {
        ...
        if (selectedRowKeys.length) {
          request(basicdata.deleteAssetsTypeList,{
            body:querystring.stringify({type:"03",assetsRecordGuids: selectedRowKeys}),
            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
            success: data => {
              if (data.status) {
                message.success("操作成功！");
                ...
                this.refs.table.fetch({...values, staticId: selectedKeys[0], type: '03',});
    ...
    if(selectedProductRowKeys.length>0){
      request(basicdata.insertAssetsType,{
        ...
        success: data => { 
          ...
          this.refs.table.fetch({type:'03',staticId:selectedKeys[0]})
        },
        error: err => {console.log(err)}
      })
    }else{
  ...
  closeProductModal = ()=>{
    const { selectedKeys } = this.state;
    this.refs.tables.fetch({type:'03',staticId:selectedKeys[0]});
...
export default Form.create()(FinanceClassify);
...
class ProductForm extends Component {
  ...
  onSearch = () =>{
    this.props.form.validateFieldsAndScroll((err, values) => {
      this.props.tableFetch({...values,staticId:this.props.staticId,type: '03'});
    })
  }
  
  modalReset = () =>{
    this.props.form.resetFields();
    this.props.tableFetch({staticId:this.props.staticId,type: '03'});
  }
  render(){
    const { getFieldDecorator } = this.props.form;
    return(
